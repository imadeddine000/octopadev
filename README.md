...
Octopadev is a local multi-provider coding assistant launcher for the bundled terminal client in this repository. It gives you one entry point and lets you choose how model requests are resolved at startup:
...
Octopadev is designed to feel like one tool rather than a provider-specific wrapper. The launcher, provider prompts, environment variables, and documentation are all centered around the `Octopadev` name.
...
octopadev/
├── src/
│   ├── launcher.js              # Main entry point - interactive provider selector
│   ├── proxy.ts                 # Anthropic-compatible proxy server (TypeScript)
│   └── types.ts                 # Shared TypeScript types
├── cli/
│   ├── package/                 # Bundled Claude Code terminal client
│   ├── package.json             # CLI package metadata
│   ├── patch-branding.js        # Branding patch for bundled client
│   └── README.md                # Bundled client documentation
├── .env.example                 # Optional environment template
├── package.json                 # Root workspace configuration
├── tsconfig.json                # TypeScript configuration
└── CLAUDE.md                    # Development guide for Octopadev
```
...
# Launch Octopadev (interactive provider selection)
npm run octopadev

# Or specify provider directly
OCTOPADEV_PROVIDER=deepseek npm run octopadev
...
Use a Deepseek API key and route requests through the local compatibility proxy.

Deepseek is an open-source friendly alternative to other commercial models with competitive pricing and fast inference.
...
These requirements apply to Octopadev itself:
...
Practical guidance for Octopadev users:
...
Editing `.env` is optional. Octopadev can prompt for missing values interactively when it starts.

## Quick Start

Start Octopadev from the repository root:
...
When Octopadev starts, it shows a provider selector:
...
If a required API key is missing, Octopadev prompts for it.

## How To Use Ollama With Octopadev
...
### 4. Start Octopadev and choose Ollama
...
Octopadev will point the bundled client at the local compatibility proxy, and the proxy will forward requests to your Ollama server.
...
- `OLLAMA_MODEL` is the model name Octopadev will request
...
You can also confirm that the Octopadev proxy is healthy:
...
```bash
mpm run octopadev --version
```

Skip the provider menu and force a specific provider:

```bash
npm run octopadev -- --provider anthropic
npm run octopadev -- --provider gemini
npm run octopadev -- --provider groq
npm run octopadev -- --provider ollama
npm run octopadev -- --provider deepseek
```

Legacy aliases are still accepted:

```powershell
.\octopadev.cmd --provider claude
.\octopadev.cmd --provider grok
```

Run a one-shot prompt:

```powershell
echo "Summarize this repository" | .\octopadev.cmd --bare -p
```
...
That usually means Octopadev is working correctly, but the local Ollama server is not reachable or does not have the requested model.
